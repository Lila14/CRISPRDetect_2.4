...
require "$cd_path/CD_MODULES/CRISPRDETECT_SUBS.pm";
use lib '$cd_path/lib';
...
my $pm = new Parallel::ForkManager($no_of_threads);      # specify the number of parallel processes CRISPRDetect should use. For dual core processor based system change it to 2.
...
my $check_direction=1;						# ---default set to 1 
	...
	#-------------------------- change the parameters for directional analysis you like ----------------------------------------------------------------------------
...
print qq~
CRISPRDetect Version:2.1 help:
Syntax:	
	perl CRISPRDetect_v2.1.pl -f test.fa -o test_CRISPRDetect                  	[ CRISPRDetect runs with all the default paramters]

	perl CRISPRDetect_v2.1.pl -f test.fa -o test_CRISPRDetect -check_direction 0	[ CRISPRDetect does not check and correct direction]

	perl CRISPRDetect_v2.1.pl -f test.fa -o test_CRISPRDetect -check_direction 0 -array_quality_score_cutoff 3 -q 1 -T 0	
	
	perl CRISPRDetect_v2.1.pl -g NZ_CP006019.gbk -o NZ_CP006019_CRISPRDetect > NC_003106_CRISPRDetect.log
...
--------------------------------------------------------	
	...
	-max_gap_between_crisprs	125	By default the maximum gap is set tp 125 nucleotides between the repeating identical words.
	-repeat_length_cutoff		17	After the intial processing, putative CRISPRs with repeat length less than this value will be rejected.


Filtering parameters [optional]:
-------------------------------	
	-minimum_repeat_length		23	This is the default directory where temporary files generated by CRISPRDetect and its dependencies will be stored.	
	-minimum_no_of_repeats		3	Predicted CRISPRs with number of repeats less than this value will be excluded. To include CRISPRs with 2 repeats, use -minimum_no_of_repeats 3
	-array_quality_score_cutoff	4	Predicted CRISPRs with score less than this value will be excluded from the output file. 
						The CRISPRs with score >=0 and less than the specied value will be moved to the inputted output.fp file [output referes to user give output file]
...
---------------------------	
	...
	To test different methods as specified in the literature, open the CRISPRDetect_v2.1.pl program with any text editor [e.g. gedit in RHEL/Fedora/CentOS, or vi in any *nix OS, or 
...
NOTE:   
	...
	CRISPRDetect_v2.1.pl should run under any unix based operating system that has a working 'perl' executable [comes with default installations under all *nix based operating systems]. 
